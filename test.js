...
const conf = {koa:{}, virtual:[ { name: 'stellar-fox.tld', root:'fake-www', default: true}, ], };
...
describe('Debug Virtual Host', () => {
  ...
  describe('default vhost behavior', () => {
    describe('when requests', () => {
      ...
      it('should 403 for folder without index.html', done => { request.get('/empty-directory').set('Host', '127.0.0.1').expect(403, done) });
...
describe('Live Virtual Host Behavior', () => {
  ...
  describe('default vhost behavior', () => {
    describe('when requests', () => {
      ...
      it('should not 403 for folder without index.html', done => { request.get('/empty-directory').set('Host', '127.0.0.1').expect('Location', "/").expect(302, done) });
      it('should redirect for dir without slash', done => { request.get('/abc').set('Host', '127.0.0.1').expect('Location', "/abc/").expect(302, done) });
    });
  });
});
